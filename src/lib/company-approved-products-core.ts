...
export function mapApprovedProductRecord(value: unknown): EstimateApprovedProduct | null {
    ...
    return {
        ...
        mainMedia: mapProductMedia(record.main_media),
        additionalMedia: Array.isArray(record.additional_media)
            ? record.additional_media.map(mapProductMedia).filter((media): media is NonNullable<EstimateApprovedProduct['mainMedia']> => Boolean(media))
            : [],
        masterPrimaryImageUrl: readNullableText(record.master_primary_image_url),
...
function mapProductMedia(value: unknown): EstimateApprovedProduct['mainMedia'] {
    if (!value || typeof value !== 'object' || Array.isArray(value)) return null;

    const record = value as Record<string, unknown>;
    const id = readText(record.id);
    const companyId = readText(record.company_id);
    const productId = readText(record.product_id);
    const bucket = readText(record.bucket);
    const storagePath = readText(record.storage_path);

    if (!id || !companyId || !productId || !bucket || !storagePath) return null;
    ...
    return {
        ...
        bucket,
        storagePath,
        altText: readNullableText(record.alt_text),
