...
export const propertyAreaCatalog: PropertyAreaCatalogCard[] = [
    ...interiorAreas.map((name) => ({ name, scope: 'interior' as const, icon: '⌂' })),
    ...exteriorAreas.map((name) => ({ name, scope: 'exterior' as const, icon: '⌂' })),
];
...
/** Conservative classification: a bare Garage and unknown names remain visible. */
export function classifyPropertyArea(area: Pick<PropertyAreaRecord, 'name' | 'area_scope'>): PropertyAreaScope {
    ...
    if (interiorAreas.some((candidate) => normalizePropertyAreaName(candidate) === name)) return 'interior';
    if (exteriorAreas.some((candidate) => normalizePropertyAreaName(candidate) === name)) return 'exterior';
    return 'unclassified';
}
...
export function catalogForScope(scope: Exclude<PropertyAreaScope, 'unclassified'>, activeRows: PropertyAreaRecord[]) {
    const activeNames = new Set(activeRows.map((row) => normalizePropertyAreaName(row.name)));
    return propertyAreaCatalog.filter((card) => card.scope === scope && !activeNames.has(normalizePropertyAreaName(card.name)));
}

export function areaSlug(name: string) {
    return normalizePropertyAreaName(name).replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '') || 'area';
}
