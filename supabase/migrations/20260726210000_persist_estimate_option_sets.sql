-- Persist the exact technician-reviewed option set used for homeowner presentation.

begin;

alter table public.company_estimate_options
    add column if not exists source_choice_id text,
    add column if not exists choice_snapshot jsonb not null default '{}'::jsonb,
    add column if not exists price_adjustment_percentage numeric(8,2) not null default 0,
    add column if not exists selected_for_presentation boolean not null default false;

create unique index if not exists company_estimate_options_session_source_choice_idx
    on public.company_estimate_options(session_id, source_choice_id)
    where source_choice_id is not null;
...
begin
    ...
    loop
        ...
        values (
            ...
            v_source_choice_id = nullif(btrim(p_selected_source_choice_id), ''),
            now()
        );
    end loop;
    ...
    if p_selected_source_choice_id is not null and v_selected_count <> 1 then
    ...
    return jsonb_build_object(
        ...
        'selected_source_choice_id', nullif(btrim(p_selected_source_choice_id), ''),
...
create or replace function public.get_company_estimate_option_set(p_session_id uuid)
returns jsonb
language plpgsql
stable
security definer
...
begin
    ...
    return jsonb_build_object(
        ...
        'technician_approved_at', v_session.technician_approved_at,
        'selected_source_choice_id', (
            select option_row.source_choice_id
        ...
        'options', coalesce((
            select jsonb_agg(
                ...
                jsonb_build_object('priceAdjustmentPercentage', option_row.price_adjustment_percentage)
...
revoke all on function public.get_company_estimate_option_set(uuid) from public, anon;
grant execute on function public.get_company_estimate_option_set(uuid) to authenticated;
