...
-- The test-only reset removes HomeOS rows before it removes the property.
-- Keep its pre-existing behavior and grants, but scope the Area hard-delete
-- exemption to that one delete instead of relying on unsupported ALTER
-- FUNCTION configuration for this custom setting.
...
declare
    ...
    v_previous_homeos_property_teardown text := coalesce(
        current_setting('barbarosa.homeos_property_teardown', true),
        ''
    );
begin
    ...
    if to_regclass('public.home_items') is not null then
        begin
            perform set_config('barbarosa.homeos_property_teardown', 'allowed', true);

            execute 'delete from public.home_items where property_id = $1'
            using v_property_id;

            perform set_config(
                'barbarosa.homeos_property_teardown',
                v_previous_homeos_property_teardown,
                true
            );
